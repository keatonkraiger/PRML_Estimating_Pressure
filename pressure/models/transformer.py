...
class STCAttention(nn.Module):
    def __init__(self, dim, num_heads=8, dropout=0.1):
        ...
        self.attn_weights_spatial = None
        self.attn_weights_temporal = None
        
        # Single QKV projection that will be split for spatial and temporal paths
        self.qkv = nn.Linear(dim, dim * 6)  # *6 because we need 2 sets of QKV (spatial + temporal)
    ...
    def forward(self, x):
        ...
        # Get QKV projections and split into spatial and temporal components
        qkv = self.qkv(x)  # B, T, J, 6*D
        qkv = rearrange(qkv, 'b t j (k h d) -> k b h t j d', k=6, h=self.num_heads)
        
        # Split into spatial and temporal QKV
        (q_s, k_s, v_s, q_t, k_t, v_t) = qkv
        
        # Spatial attention (across joints for each timestamp)
        # Reshape for spatial attention computation
        q_s = q_s.transpose(1, 2)  # B, H, T, J, D
        ...
        self.attn_weights_spatial = attn_s.mean(dim=1).detach()  # B, T, J, J
        spatial_out = (attn_s @ v_s)  # B, H, T, J, D
        
        # Temporal attention (across timestamps for each joint)
        # Reshape for temporal attention computation
        q_t = q_t.transpose(2, 3)  # B, H, J, T, D
        k_t = k_t.transpose(2, 3)
        v_t = v_t.transpose(2, 3)
        
        attn_t = (q_t @ k_t.transpose(-2, -1)) * self.scale
        attn_t = F.softmax(attn_t, dim=-1)
        # Store attention weights - average across heads for visualization
        self.attn_weights_temporal = attn_t.mean(dim=1).detach()  # B, J, T, T
        temporal_out = (attn_t @ v_t)  # B, H, J, T, D
        
        # Combine spatial and temporal attention results
        spatial_out = spatial_out.transpose(1, 2)  # B, T, H, J, D
        temporal_out = temporal_out.transpose(2, 3)  # B, T, H, J, D
        
        # Merge heads and combine spatial and temporal features
        breakpoint()
        out = rearrange([spatial_out, temporal_out], 'n b t h j d -> b t j (n h d)', h=self.num_heads)
...
class STCTransformerBlock(nn.Module):
    def __init__(self, dim, num_heads=8, mlp_ratio=4.0, dropout=0.1):
        super().__init__()
        self.attn = STCAttention(dim, num_heads, dropout)
...
class STCTransformer(nn.Module):
    def __init__(self, transformer_dim, num_heads, num_layers, seq_len, dropout_p=0.1,
        ...
        self.layers = nn.ModuleList([
            STCTransformerBlock(
                dim=transformer_dim,
                num_heads=num_heads,
                mlp_ratio=mlp_dim/transformer_dim,
                dropout=dropout_p
    ...
    def forward(self, x, return_attention=False):
        # x shape: (batch_size, seq_len, num_joints, transformer_dim)
        
        # Pass through transformer blocks
        for i, layer in enumerate(self.layers):
            x = layer(x)
            
            # Store attention weights from the last layer if requested
            if return_attention and i == len(self.layers) - 1:
                spatial_attn = layer.attn.attn_weights_spatial
                temporal_attn = layer.attn.attn_weights_temporal
    ...
    def get_last_attention_weights(self):
        ...
        return {
            'spatial_attention': last_layer.attn.attn_weights_spatial,
            'temporal_attention': last_layer.attn.attn_weights_temporal
        }
...
from pressure.models.transformers import STCTransformer
...
class FootFormer(nn.Module):
    def __init__(self, num_joints, joint_dim, pose_embed_dim, num_heads, num_layers, output_dims, seq_len=5, dropout_p=0.1, transformer='transformer', 
        ...
        elif transformer == 'STCFormer':
            self.transformer = STCTransformer(
                transformer_dim=pose_embed_dim,
                num_heads=num_heads,
                num_layers=num_layers,
                seq_len=seq_len,
